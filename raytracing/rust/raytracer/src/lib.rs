...
impl PPM {
    ...
    pub fn gradient(width: usize, height: usize) -> PPM {
        ...
        for i in 0..height {
            for j in 0..width {
                let r = (255.0 * i as f32 / (height - 1) as f32) as usize;
                let g = (255.0 * j as f32 / (width - 1) as f32) as usize;
                let b = 0;
                data.push(RGB(r, g, b));
            }
        }
    ...
    pub fn to_string(&self) -> String {
        ...
        for (i, pixel) in self.data.iter().enumerate() {
            buffer.push_str(&format!("{} {} {}", pixel.0, pixel.1, pixel.2));
            if (i + 1) % self.width == 0 {
                buffer.push('\n');
            } else if (i + 1) != self.data.len() {
                buffer.push(' ');
            }
        }
        buffer
